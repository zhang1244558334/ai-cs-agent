...
def qrcode_login(poll_interval: float = 3.0, timeout: float = 120.0,
    ...
    # 保存二维码图片（备用）
    try:
        import qrcode as _qr_lib, shutil as _sh
        _img = _qr_lib.make(qr_url)
        _img = _img.resize((400, 400))
        _img.save('/tmp/xy_qr.png')
        _sh.copy('/tmp/xy_qr.png', os.path.join(os.path.expanduser('~'), '桌面', '闲鱼扫码.png'))
        print('[qrcode_login] 二维码已保存到桌面: 闲鱼扫码.png (400x400)')
    ...
    while time.time() < deadline:
        ...
        if status == 'CONFIRMED':
            ...
            print(f'[qrcode_login] CONFIRMED! qdata keys: {list(qdata.keys())}')
            print(f'[qrcode_login] login_token: {login_token}')
            print(f'[qrcode_login] full qdata: {json.dumps(qdata, ensure_ascii=False)}')
            # 打印 Set-Cookie
            set_cookie = resp.headers.get('Set-Cookie', '')
            print(f'[qrcode_login] Set-Cookie: {set_cookie[:500]}')
            
            # —— 新区块：尝试跟随 iframeRedirectUrl ——
            iframe_url = qdata.get('iframeRedirectUrl', '')
            if iframe_url:
                print(f'[qrcode_login] 跟随 iframeRedirectUrl: {iframe_url}')
                try:
                    r = s.get(iframe_url, headers={'User-Agent': UA, 'Referer': 'https://passport.goofish.com/mini_login.htm'}, timeout=15, allow_redirects=True)
                    print(f'[qrcode_login] iframe 响应: status={r.status_code}')
                    if r.history:
                        for h in r.history:
                            print(f'[qrcode_login]   重定向: {h.status_code} → {h.url}')
                    
                    # 再访问 goofish.com 首页，触发登录 cookie
                    print(f'[qrcode_login] 访问 goofish.com 首页...')
                    r2 = s.get('https://www.goofish.com/', headers={'User-Agent': UA}, timeout=15, allow_redirects=True)
                    print(f'[qrcode_login] goofish.com 响应: status={r2.status_code}')
                    
                    # 再试一次登录页
                    print(f'[qrcode_login] 再次访问 mini_login...')
                    r3 = s.get('https://passport.goofish.com/mini_login.htm',
                               params={'lang': 'zh_cn', 'appName': 'xianyu', 'appEntrance': 'web',
                                       'styleType': 'vertical', 'bizParams': '', 'notLoadSsoView': 'false',
                                       'notKeepLogin': 'false', 'isMobile': 'false'},
                               headers={'User-Agent': UA, 'Referer': 'https://www.goofish.com/'},
                               timeout=15, allow_redirects=True)
                    print(f'[qrcode_login] mini_login 响应: status={r3.status_code}')
                    
                    # 检查 cookie 变化
                    unb = s.cookies.get('unb', domain='.goofish.com')
                    sgcookie = s.cookies.get('sgcookie', domain='.goofish.com')
                    tracknick = s.cookies.get('tracknick', domain='.goofish.com')
                    print(f'[qrcode_login] 最终: unb={unb}, sgcookie={sgcookie}, tracknick={tracknick}')
                    # 打印所有 goofish.com cookie
                    goofish_cookies = {c.name: c.value for c in s.cookies if '.goofish.com' in (c.domain or '')}
                    print(f'[qrcode_login] goofish cookies ({len(goofish_cookies)}): {list(goofish_cookies.keys())}')
                except Exception as e:
                    print(f'[qrcode_login] iframe 请求失败: {e}')
                    import traceback
                    traceback.print_exc()
            break
        elif status == 'EXPIRED':
    ...
    if not login_token:
        ...
        print(f'[qrcode_login] 无 login_token，检查 cookie...')
        unb = s.cookies.get('unb')
        sgcookie = s.cookies.get('sgcookie')
        print(f'[qrcode_login] unb={unb}, sgcookie={sgcookie}')
        if unb:
