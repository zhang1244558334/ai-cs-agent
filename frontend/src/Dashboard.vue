...
function renderCharts() {
  ...
  nextTick(() => {
    ...
    // 饼图 - 意图分布
    if (pieChart.value) {
      pieInstance = echarts.init(pieChart.value, isDark ? null : null, {
        // not using theme, just explicitly set styles
      })
    ...
    if (lineChart.value) {
      ...
      lineInstance.setOption({
        ...
        yAxis: {
          ...
          axisLine: { lineStyle: { color: textColor } },
          splitLine: { lineStyle: { color: isDark ? 'rgba(255,255,255,0.06)' : '#eee' } },
        },
        series: [{
          type: 'line',
...
onMounted(() => {
  ...
  window.addEventListener('dark-mode-changed', () => { renderCharts() })
...
.stat-card:nth-child(1) { border-top-color: #67c23a; }
.stat-card:nth-child(2) { border-top-color: #409eff; }
.stat-card:nth-child(3) { border-top-color: #e6a23c; }
.stat-card:nth-child(4) { border-top-color: #f56c6c; }
.stat-card:hover {
...
.layout.dark .stat-icon.health  { background: rgba(82, 196, 26, 0.12); color: #73d13d; }
.layout.dark .stat-icon.primary { background: rgba(24, 144, 255, 0.12); color: #40a9ff; }
.layout.dark .stat-icon.warning { background: rgba(250, 140, 22, 0.12); color: #ffa940; }
.layout.dark .stat-icon.danger  { background: rgba(255, 77, 79, 0.12); color: #ff7875; }
